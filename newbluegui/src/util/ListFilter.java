...
public class ListFilter<Job> extends ArrayList<Job> {
    ...
    public void filterClient(Client f) {
        ...
        for (Job j : this) {
            if (((job.Job) j).getClient().getId() == f.getId())
                aux.add(j);
        }
    ...
    public void filterJobType(JobType f) {
        ...
        for (Job j : this) {
            if (((job.Job) j).getJobPrice().getJobType().getId() == f.getId())
                aux.add(j);
        }
    ...
    public void filterProductColor(ProductColor f) {
        ...
        for (Job j : this) {
            if (((job.Job) j).getProductColor().getId() == f.getId())
                aux.add(j);
        }
    ...
    public void filterInitDate(LocalDate f) {
        ...
        for (Job j : this) {
            if (((job.Job) j).getDate().after(d) || ((job.Job) j).getDate().equals(d))
                aux.add(j);
        }
    ...
    public void filterEndDate(LocalDate f) {
        ...
        for (Job j : this) {
            if (((job.Job) j).getDate().before(d) || ((job.Job) j).getDate().equals(d))
                aux.add(j);
        }
